<script lang="ts">
  ...
  const columnsWithComponents: ComponentProps<
  ...
  const tableColumns: ComponentProps<
    BasicTable<(typeof transactionEntries)[number]>
  >['columns'] = [
    null,
    {
      header: {
  ...
  const simpleTableColumns: ComponentProps<
