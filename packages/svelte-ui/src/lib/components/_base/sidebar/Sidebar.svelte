<script lang="ts">
	...
	import { VIEW_PORTS } from '../../../../../../common/src/utils/constants.js';
	import { showSidebar } from '../../sidebar-with-navbar/index.js';
	...
	$: {
		if (showMobileSidebar && !isDesktopViewport) document.body.classList.add('no-scroll');
...
{#if showMobileSidebar && !isDesktopViewport}
	<nav bind:clientWidth={width} transition:fly|global={{ x: -width, opacity: 1, duration: 300 }}>
		<button
			class="close"
