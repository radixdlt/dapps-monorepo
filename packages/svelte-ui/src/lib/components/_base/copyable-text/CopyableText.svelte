<script lang="ts">
	...
	import { copyToClipboard } from '@svelte-ui/directives/copy-to-clipboard';
	...
	const checkOverflow = (child: HTMLElement, ancestor: HTMLElement) => {
		const { left: leftThreshold, right: rightThreshold } = child.getBoundingClientRect();
	...
	const handleResize = () => {
		if (short) {
			...
			short = left > resizeToLongThreshold.left || right < resizeToLongThreshold.right;
		} else {
