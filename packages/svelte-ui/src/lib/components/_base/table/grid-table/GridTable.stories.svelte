...
<Story name="Primary">
	<GridTable {columns}>
		...
		<div slot="header-cell" let:column let:sort on:click={() => (rows = sort()(rows))}>
			{column?.header?.label}
		</div>
		...
		<svelte:fragment slot="rows" let:Section>
			{#each rows as row}
				<Section {columnIds}>
					...
					>
						<div class="favorites">
							<Section {columnIds} renderAt="favoriteColor">{row.favoriteColor}</Section>
							<Section {columnIds} renderAt="favoriteAnimal">{row.favoriteAnimal}</Section>
						</div>
					</Section>
				</Section>
			{/each}
