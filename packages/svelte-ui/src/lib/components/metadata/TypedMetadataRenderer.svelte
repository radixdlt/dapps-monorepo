<script lang="ts">
	...
	import LinksList from '@svelte-ui/components/_base/link/LinksList.svelte';
	import type { MetadataTypedValue } from '@common/utils/gateway-sdk';
...
{#if metadataTypedValue}
	...
	{:else if metadataTypedValue.type === 'NonFungibleGlobalId'}
		<AddressesList
			autoShorten
			addresses={[`${metadataTypedValue.resource_address}:${metadataTypedValue.non_fungible_id}`]}
		/>
	...
	{:else if metadataTypedValue.type === 'OriginArray'}
		<LinksList links={metadataTypedValue.values.map((url) => ({ url }))} external />
	...
	{:else if metadataTypedValue.type === 'PublicKeyArray'}
		{metadataTypedValue.values.map((value) => `${value.key_type}(${value.key_hex})`).join(', ')}
	{:else if metadataTypedValue.type === 'PublicKeyHash'}
		{metadataTypedValue.value.key_hash_type}({metadataTypedValue.value.hash_hex})
