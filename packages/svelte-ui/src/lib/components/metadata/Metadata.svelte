...
<script lang="ts">
	...
	import type { EntityMetadataItem, EntityMetadataItemValue } from '@common/utils/gateway-sdk';
	...
	export let metadata: EntityMetadataItem[] | Promise<EntityMetadataItem[] | void>;

	$: entries = Promise.resolve(metadata).then((resolved) => indexBy(prop('key'), resolved || []));
</script>
