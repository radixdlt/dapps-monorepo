...
if (!process.env.PUBLIC_NETWORK_NAME) {
    console.error('ERROR: Please specify a network to update the gateway mocks for')
    process.exit(1)
}

child_process.execSync('yarn add @radixdlt/babylon-gateway-api-sdk')

const GATEWAY_SCHEMA_URL = `${NETWORK_CONFIG[process.env.PUBLIC_NETWORK_NAME]!.url}/gateway-api-schema.json`

JSONSchemaFaker.option({ alwaysFakeOptionals: true, useExamplesValue: true, minItems: 3 })

const fakeResponse = (schema: any) => (path: string) => {
    const method = schema.paths[path].post
    return JSONSchemaFaker.generate(method.responses['200'].content['application/json'].schema as any, {})
}
