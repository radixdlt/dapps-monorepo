<script lang="ts">
  ...
  import { accountLabel, fetchWrapper } from '@utils'
      ...
      async (requestData) => {
        await createChallenge()
          ...
          .andThen(({ signedChallenges, accounts }) =>
            login(signedChallenges[0])
              .map(() => {
                updateAccounts(accounts)
              })
              .andThen(() => bookmarkedValidatorsApi.create('abc123'))
              .andThen(() => bookmarkedValidatorsApi.remove('abc123'))
          )
      },
      {
        explorer: {
